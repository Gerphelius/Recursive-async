...
async function startRecursive() {
  if (users.length) {
    const usersToFetch = users.splice(users.length - 10, 10);
    const processedUsers = usersToFetch.map(user => processUser(user.id));
    const fetchedUsers = await Promise.all(processedUsers);

    return await startRecursive() + fetchedUsers.reduce((sum, current) => sum + current, 0);
  } else {
    return 0;
  };
}
